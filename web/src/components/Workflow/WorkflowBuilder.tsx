...
const CustomHandle = ({
  ...
  const color = id == "failure" ? "bg-red" : "bg-blue";
  console.log(color);
    ...
    <Handle type={type} position={position} id={id}>
      {isHovered && (
        <div className=" left-1/2 -bottom-10 flex flex-col items-center">
          ...
          <button
            className={`${color}-500 text-white rounded-full w-6 h-6 flex items-center justify-center shadow-md hover:${color}-700 cursor-pointer`}
            onClick={(e) => {
...
const SwitchNode: React.FC<NodeProps> = ({ data }) => (
  ...
  >
    ...
    <div className="text-xs">
      {(data as SwitchNodeData).key
        ? `switch(${(data as SwitchNodeData).key})`
        : "Click to configure"}
    </div>
    {(data as SwitchNodeData).cases.map((caseValue, index) => (
      <Handle
        ...
        id={`case-${caseValue}`}
        style={{
          left: `${((index + 1) * 100) / ((data as SwitchNodeData).cases.length + 1)}%`,
        }}
      />
    ))}
  </div>
...
const ResponseNode: React.FC<NodeProps> = ({ data }) => (
  ...
  >
    ...
    <div className="font-bold">
      {(data as ResponseNodeData).type === "success"
        ? "Success Response"
        : "Error Response"}
    </div>
...
const EdgeTypesFlow = () => {
  ...
  const [choosenNodeType, setChoosenNodeType] = useState<
    "condition" | "switch" | "response" | null
  >(null);
  const initialEdges = [
    {
      type: "customEdge",
